<script lang="ts">
    import Draggable from "$packages/svelte-draggable/Draggable.svelte";
    import DraggableChild from "$packages/svelte-draggable/DraggableChild.svelte";  
    ...
    {#each images as image,index }
        <DraggableChild on:dropEnded={handleDrop} id={index}>
            <div class="test">
                ...
                <div class="icon2 exclude" on:click={()=>alert("HELLO")} on:keypress={()=>alert("HELLO")}></div>
            </div>
            <Image src={image.src}/>
        </DraggableChild>
    {/each}
